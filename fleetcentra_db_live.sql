CREATE DATABASE IF NOT EXISTS `fleetcentra_db_live` DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
USE `fleetcentra_db_live`;

/*
DROP TABLE IF EXISTS `fleetcentra_admin_settings`;
CREATE TABLE IF NOT EXISTS `fleetcentra_admin_settings` (
...
TRUNCATE TABLE `fleetcentra_admin_settings`;
*/


DROP TABLE IF EXISTS `fleetcentra_users`;
CREATE TABLE IF NOT EXISTS `fleetcentra_users`(
...
TRUNCATE TABLE `fleetcentra_users`;

INSERT INTO `fleetcentra_users` VALUES (1,'19910509','','Sai Deepak',NULL,'C',NULL,NULL,NULL,'','sai@livewd.ca','sai@livewd.ca',9,'$2y$10$8cgXF8pQ8oYRcqiu1xIGTu22HUbjEhINHgdit.pv.OAGeHdBv4thC',NULL,NULL,'images/avatar.png',NULL,NULL,NULL,'CA',NULL,NULL,NULL,'','',NULL,NULL,'y8j~m2VKif3l%BaN]UJls','','2026-02-20 06:03:41','209.121.189.20',1,NULL,'2026-02-20 06:03:41','2026-02-20 06:03:41',0,0,'','',NULL,NULL,NULL);

DROP TABLE IF EXISTS `fleetcentra_tenants`;
CREATE TABLE IF NOT EXISTS `fleetcentra_tenants` (
...
INSERT INTO fleetcentra_tenants (
    tenant_id,
    tenant_name,
    tenant_code,
    contact_email,
...
UPDATE fleetcentra_tenants SET tenant_id = 20000000 WHERE tenant_id = 1 AND tenant_id < 20000001;

ALTER TABLE fleetcentra_users
ADD CONSTRAINT fk_users_default_tenant
FOREIGN KEY (default_tenant_id)
REFERENCES fleetcentra_tenants(tenant_id)
ON DELETE RESTRICT;

DROP TABLE IF EXISTS `fleetcentra_user_tenants`;
CREATE TABLE IF NOT EXISTS `fleetcentra_user_tenants` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  -- Foreign keys
  ...
  CONSTRAINT `fk_ut_user`
    FOREIGN KEY (`user_id`) REFERENCES `fleetcentra_users`(`id`)
    ON DELETE CASCADE,
  CONSTRAINT `fk_ut_tenant`
    FOREIGN KEY (`tenant_id`) REFERENCES `fleetcentra_tenants`(`tenant_id`)
    ON DELETE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;


DROP TABLE IF EXISTS `fleetcentra_user_roles`;
CREATE TABLE IF NOT EXISTS `fleetcentra_user_roles` (
...
ALTER TABLE fleetcentra_user_tenants
ADD CONSTRAINT fk_ut_role
FOREIGN KEY (role_id)
REFERENCES fleetcentra_user_roles(role_id)
ON DELETE RESTRICT;

INSERT INTO fleetcentra_user_tenants (user_id, tenant_id, role)
VALUES (1, 19910509, 'MASTERSUPERADMIN');

INSERT INTO fleetcentra_user_roles 
...
UPDATE fleetcentra_user_tenants
SET role_id = (
    SELECT role_id 
    FROM fleetcentra_user_roles 
    WHERE role_key = 'MASTERSUPERADMIN'
)
WHERE user_id = 1 AND tenant_id = 19910509;


DROP TABLE IF EXISTS `fleetcentra_activityaudit`;
CREATE TABLE IF NOT EXISTS `fleetcentra_activityaudit` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
...
TRUNCATE TABLE `fleetcentra_activityaudit`;

DROP TABLE IF EXISTS `fleetcentra_emailtemplates`;
CREATE TABLE IF NOT EXISTS `fleetcentra_emailtemplates` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
...
TRUNCATE TABLE `fleetcentra_emailtemplates`;

INSERT IGNORE INTO `fleetcentra_emailtemplates` (`id`, `index_key`, `template_code`, `active`) VALUES
...
DROP TABLE IF EXISTS `fleetcentra_useractivityaudit`;
CREATE TABLE IF NOT EXISTS `fleetcentra_useractivityaudit` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
TRUNCATE TABLE `fleetcentra_useractivityaudit`;

DROP TABLE IF EXISTS `fleetcentra_permissions`;
CREATE TABLE IF NOT EXISTS `fleetcentra_permissions` (
...
TRUNCATE TABLE `fleetcentra_permissions`;

INSERT INTO `fleetcentra_permissions` 
...
DROP TABLE IF EXISTS `fleetcentra_role_permissions`;
CREATE TABLE IF NOT EXISTS `fleetcentra_role_permissions` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  -- Foreign keys
  ...
  CONSTRAINT `fk_rp_role`
    FOREIGN KEY (`role_id`) REFERENCES `fleetcentra_user_roles`(`role_id`)
    ON DELETE CASCADE,
  CONSTRAINT `fk_rp_permission`
    FOREIGN KEY (`permission_id`) REFERENCES `fleetcentra_permissions`(`permission_id`)
    ON DELETE CASCADE,
  -- Optional: track who made changes
  CONSTRAINT `fk_rp_created_by`
    FOREIGN KEY (`created_by`) REFERENCES `fleetcentra_users`(`id`)
    ON DELETE SET NULL,
  CONSTRAINT `fk_rp_updated_by`
    FOREIGN KEY (`updated_by`) REFERENCES `fleetcentra_users`(`id`)
    ON DELETE SET NULL,
  CONSTRAINT `fk_rp_deleted_by`
    FOREIGN KEY (`deleted_by`) REFERENCES `fleetcentra_users`(`id`)
    ON DELETE SET NULL
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;


DROP TABLE IF EXISTS `fleetcentra_audit_logs`;
CREATE TABLE IF NOT EXISTS `fleetcentra_audit_logs` (
...
DROP TABLE IF EXISTS `fleetcentra_audit_settings`;
CREATE TABLE IF NOT EXISTS `fleetcentra_audit_settings` (
...
---------------------------------------------------------------------------
INSERT INTO fleetcentra_audit_logs
...
sql
DELETE FROM fleetcentra_audit_logs
WHERE created_on < NOW() - INTERVAL (
    SELECT retention_days FROM fleetcentra_audit_settings WHERE tenant_id IS NULL
) DAY;
+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
...
sql
DELETE l
FROM fleetcentra_audit_logs l
JOIN fleetcentra_audit_settings s ON s.tenant_id = l.tenant_id
WHERE l.created_on < NOW() - INTERVAL s.retention_days DAY;
*/

DROP TABLE IF EXISTS `fleetcentra_countries`;
CREATE TABLE IF NOT EXISTS `fleetcentra_countries` (
...
TRUNCATE TABLE `fleetcentra_countries`;

INSERT INTO fleetcentra_countries (country_code, country_name, phone_code) VALUES
('US','United States','+1'),
('CA','Canada','+1'),
('MX','Mexico','+52'),
...
DROP TABLE IF EXISTS `fleetcentra_currencies`;
CREATE TABLE IF NOT EXISTS `fleetcentra_currencies` (
...
TRUNCATE TABLE `fleetcentra_currencies`;

INSERT INTO fleetcentra_currencies (currency_code, currency_name, symbol) VALUES
('USD','US Dollar','$'),
('CAD','Canadian Dollar','$'),
('MXN','Mexican Peso','$'),
('EUR','Euro','€'),
...
DROP TABLE IF EXISTS `fleetcentra_timezones`;
CREATE TABLE IF NOT EXISTS `fleetcentra_timezones` (
...
TRUNCATE TABLE `fleetcentra_timezones`;

INSERT INTO fleetcentra_timezones (timezone_name, utc_offset) VALUES
...
DROP TABLE IF EXISTS `fleetcentra_languages`;
CREATE TABLE IF NOT EXISTS `fleetcentra_languages` (
...
TRUNCATE TABLE `fleetcentra_languages`;

DROP TABLE IF EXISTS `fleetcentra_system_settings`;
CREATE TABLE IF NOT EXISTS `fleetcentra_system_settings` (
...
TRUNCATE TABLE `fleetcentra_system_settings`;

DROP TABLE IF EXISTS `fleetcentra_password_resets`;
CREATE TABLE IF NOT EXISTS `fleetcentra_password_resets` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
  `user_id` BIGINT UNSIGNED NOT NULL,
...
TRUNCATE TABLE `fleetcentra_password_resets`;

DROP TABLE IF EXISTS `fleetcentra_config`;
CREATE TABLE IF NOT EXISTS `fleetcentra_config` (
  `id` BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
...
TRUNCATE TABLE `fleetcentra_config`;

INSERT INTO `fleetcentra_config` (`id`, `tenant_id_counter`) VALUES (1, 90000) ON DUPLICATE KEY UPDATE `tenant_id_counter` = `tenant_id_counter`;